...
import {
    ...
    Sparkles, AlertTriangle, ChevronRight, Wallet, LogOut, Share2, Smartphone, Briefcase, Target, RefreshCw
} from 'lucide-react';
            ...
            <div className="flex-1 flex flex-col bg-white md:rounded-2xl overflow-hidden md:border border-slate-200 shadow-sm relative h-[calc(100vh-140px)]">
                ...
                <div className="bg-white border-t border-slate-200 p-3 shrink-0">
                    <div className="flex gap-2 mb-3 overflow-x-auto pb-1 no-scrollbar">
                        <button onClick={() => handleAction('moncash')} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-700 rounded-full text-xs font-bold border border-red-100 hover:bg-red-100 transition-colors whitespace-nowrap"><Wallet className="w-3 h-3" /> {t('moncash_htg') || 'MonCash (HTG)'}</button>
                        <button onClick={() => handleAction('stripe')} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-full text-xs font-bold border border-blue-100 hover:bg-blue-100 transition-colors whitespace-nowrap"><CreditCard className="w-3 h-3" /> {t('stripe_usd') || 'Stripe (USD)'}</button>
                        <button onClick={() => setIsMapOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-700 rounded-full text-xs font-bold border border-green-100 hover:bg-green-100 transition-colors whitespace-nowrap"><MapPin className="w-3 h-3" /> {t('location_picker_title') || 'Ma position'}</button>
