...
const ZabelyPlus = () => {
    ...
    const handleSubscribe = async () => {
        ...
        try {
            ...
            await updateDoc(userRef, {
                isZabelyPlus: true,
                zabelyPlusSubscriptionDate: new Date().toISOString(),
                zabelyPlusExpiryDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toISOString()
            });
            ...
            <div className="container mx-auto px-4 py-12 text-center">
                <div className="bg-green-50 border border-green-200 rounded-xl p-8 max-w-2xl mx-auto">
                    ...
                    <div className="text-sm text-gray-500">
                        {t('member_since')} {new Date(currentUser.zabelyPlusSubscriptionDate).toLocaleDateString()}
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <>
            <SEO title="Zabely Plus" description="Abonnement premium: livraison rapide, avantages exclusifs et économies." />
                        ...
                        {[1, 2, 3, 4].map((item) => (
                            <div key={item} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden relative opacity-75">
                                <div className="absolute inset-0 flex items-center justify-center bg-black/5 z-10">
                                    <span className="bg-[#FFC400] text-primary text-xs font-bold px-2 py-1 rounded shadow-sm">{t('zabely_prestige_reserved')}</span>
                                </div>
